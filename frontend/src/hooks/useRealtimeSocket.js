...
export function useRealtimeSocket() {
  ...
  useEffect(() => { browserNotificationsEnabled.current = Boolean(preferences.browser); }, [preferences.browser]);
  useEffect(() => {
    ...
    const onPresenceUpdated = (payload) => dispatch(presenceUpdated(payload));
    const eventHandlers = new Map();
    socket.on('connect', onConnect); socket.on('disconnect', onDisconnect); socket.on('connect_error', onError); socket.on(SOCKET_EVENTS.ERROR, onError); socket.on(SOCKET_EVENTS.PRESENCE_UPDATED, onPresenceUpdated);
    EVENTS.forEach((eventName) => {
      ...
      if (browserNotificationsEnabled.current) showBrowserNotification(notification, () => window.location.assign('/dashboard/notifications'));
    ...
    socket.connect();
    return () => { eventHandlers.forEach((handler, eventName) => socket.off(eventName, handler)); socket.off('connect', onConnect); socket.off('disconnect', onDisconnect); socket.off('connect_error', onError); socket.off(SOCKET_EVENTS.ERROR, onError); socket.off(SOCKET_EVENTS.PRESENCE_UPDATED, onPresenceUpdated); };
  }, [dispatch, token]);
}
