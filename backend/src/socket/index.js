...
const configureRedisAdapter = async (server) => {
  ...
  try {
    ...
    logger.error('Socket.IO Redis adapter configuration failed', { err });
    throw err;
  }
};
...
const init = async (httpServer) => {
  io = new Server(httpServer, { cors: { origin: config.socket.corsOrigin }, pingTimeout: config.socket.pingTimeoutMs, pingInterval: config.socket.pingIntervalMs, connectTimeout: config.socket.connectTimeoutMs });
...
const broadcast = (name, payload = {}, options = {}) => {
  ...
  const now = Date.now();
  for (const [k, expires] of recentEvents) if (expires < now) recentEvents.delete(k);
  ...
  try {
    ...
    return true;
  } catch (err) { logger.error('Socket broadcast failure', { event: name, err }); return false; }
};
