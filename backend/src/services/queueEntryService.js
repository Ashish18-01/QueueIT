...
exports.join = async (queueId, data, user, req) => {
  ...
  const entry = await entryRepo.create({ organizationId: queue.organizationId, branchId: queue.branchId, venueId: queue.venueId, queueId: queue._id, customerId, tokenNumber, token: tokenFor(queue, tokenNumber), metadata: data.metadata });
...
exports.leave = async (id, user, req) => {
  ...
  const updated = await entryRepo.update(id, tenantFromEntry(entry), { status: 'left', leftAt: new Date() });
...
exports.cancel = async (id, user, req) => {
  ...
  const updated = await entryRepo.update(id, tenantFromEntry(entry), { status: 'cancelled', cancelledAt: new Date(), cancelledBy: user._id });
